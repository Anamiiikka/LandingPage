"use client";
   ...
   export default function Chatbot() {
     ...
     // Fetch initial messages when chatbot opens
     useEffect(() => {
       if (isOpen && userId) {
         const fetchMessages = async () => {
           try {
             const response = await fetch(`/api/chatbot/messages?userId=${userId}`);
     ...
     const saveMessage = async (message: ChatMessage) => {
       try {
         ...
         if (!response.ok) {
           console.error("Error saving message:", await response.json());
         }
       } catch (error) {
...
You'll receive a confirmation email with meeting details and preparation materials shortly. We're excited to work with you!`,
           ...
           console.error("Error saving booking:", await response.json());
...
The proposal will include project scope, timeline, pricing, and next steps.`,
           ...
           console.error("Error saving quote:", await response.json());
